...
private:
  Type *array;              // points to the first element of an array of T
  int size;               // size of array
  void swap(Type&, Type&); // swaps the values of two elements in array
public:
...
template <class Type> 
void Vector<Type>::Vector::swap(Type& a, Type& b)
{
	Type tmp = a;
	a = b;
	b = tmp;
...
//post 
template <class Type> 
Type Vector<Type>::Vector::VectIter::operator++()
{
...
// pre fix
template <class Type> 
Type Vector<Type>::Vector::VectIter::operator++(int){
  int temp = index;
  index += 1;
  if (index > v->size){
    index = 0;
...
template <class Type> 
Vector<Type>::Vector::VectIter::VectIter(Vector& x)
{
  v = &x;
  index = 0;
}
...
template <class Type> 
Vector<Type>::Vector::Vector(int sz)
{
  size=sz;
  array = new Type [sz];
  assert (array != NULL);
...
template <class Type> 
Type& Vector<Type>::Vector ::operator [] (int i)
{
  return array[i];
}
