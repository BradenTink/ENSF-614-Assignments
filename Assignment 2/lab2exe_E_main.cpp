...
int main(void)
{
  ...
  cplx difference;
  cplx * dif_pot = &difference;
  cplx product;
  cplx * prod_pot = &product;
  ...
  cplx_subtract(w, z, dif_pot);
  cout << "\nsum is (" << difference.real << ") + j(" << difference.imag << "}\n";
  
	
 cplx_multiply(w_pot, z_pot, prod_pot);
 cout << "\nsum is (" << product.real << ") + j(" << product.imag << "}\n";
